...
size_t da_size(DynArray* arr) {
	return (arr->el_size * arr->len);
}

size_t da_capacity(DynArray* arr) {
	return (arr->el_size * arr->capacity);
}

size_t da_len(DynArray* arr) {
	return (arr->len);
}

size_t da_el_size(DynArray* arr) {
	return (arr->el_size);
}
...
void da_append(DynArray* arr, void* value) {
	/*
		Adds the passed element to the end of the array. Assumes its size
		matches arr->el_size as initialized. Value received as pointer, value
		is copied to the end of the array.
	*/
	...
	memcpy(
		(char*)arr->data + (arr->len * arr->el_size),
		value,
		arr->el_size
	);
	arr->len++;
...
void da_remove(DynArray* arr, unsigned int index) {
	...
	*/
	...
	for (size_t i = index; i < (arr->len - 1); i++) {
		...
		memmove(
			((char*)arr->data + (i * arr->el_size)),
			((char*)arr->data + ((i + 1) * arr->el_size)),
			arr->el_size
		);
	}
	arr->len--;
...
void da_set(DynArray* arr, unsigned int index, void* newValue) {
	...
	*/
	...
	memcpy(
		(char*)arr->data + (index * arr->el_size),
		newValue,
		arr->el_size
	);
}
...
void da_pop(DynArray* arr, void* dest) {
	/*
		...
		da_append(). If we require they pass a pointer themselves, then we
		define that behavior out of existence.

		If array has no elements, we return.
	*/
